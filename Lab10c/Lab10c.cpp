...
// which has been cleaned up and improved
// since Lab7a.
//
...
{
	// the behemoth function
	// 1. takes `current_loop` to determine data type
	// 2. takes `current_index` to determine array position
	// 3. converts `current_datacache.currentPoint` to the required data point in `current_dataset`
	//    - each conversion is hardcoded, as `current_datacache.currentType` was ultimately not used
	switch (current_loop)
	{
	case 0:
...
int main()
{
	...
	{
		cout << "[ERROR: Unable to locate file \"" << file_name << "\". File must be in same location as application]\n";
		return 1; // error! [EXIT PRG]
	}

	// read and extract data from data file
	int index_y = 0;
	...
	// do some math and display those results
